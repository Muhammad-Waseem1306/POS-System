    ...
    <section class="invoice">
      ...
      <div class="row invoice-info mb-2">
        ...
        <div class="col-sm-5 invoice-col">
          ...
          @endif
          @if($order->installmentPlan && $order->installmentPlan->guarantor)
          <address>
            <strong>Guarantor: {{$order->installmentPlan->guarantor->name ?? 'N/A'}}</strong><br>
            Phone: {{$order->installmentPlan->guarantor->phone ?? 'N/A'}}<br>
            CNIC: {{$order->installmentPlan->guarantor->cnic ?? 'N/A'}}
          </address>
          @endif
        </div>
        <!-- /.col -->
        ...
        <div class="col-sm-3 invoice-col">
          ...
          Sale Type: {{ ucfirst($order->sale_type ?? 'cash') }}<br>
          @if($order->sale_type === App\Models\Order::SALE_TYPE_INSTALLMENT && $order->installmentPlan)
          Installment Months: {{$order->installmentPlan->installment_months}}<br>
          Monthly Installment: {{ (currency() ? currency()->symbol : '$') . ' ' . number_format((float)$order->installmentPlan->monthly_installment,2,'.',',')}}<br>
          @endif
        </div>
          <!-- <br>
          ...
          <table class="table table-modern table-striped">
            <thead>
              <tr>
                ...
                <th>Subtotal {{currency() ? currency()->symbol : '$'}}</th>
              </tr>
            </thead>
            <tbody>
          ...
          <div class="table-responsive">
            <table class="table">
              <tr>
                ...
                <td class="text-right">{{ (currency() ? currency()->symbol : '$') . ' ' . number_format((float)$order->sub_total,2,'.',',')}}</td>
              </tr>
              <tr>
                <th>Discount:</th>
                <td class="text-right">{{ (currency() ? currency()->symbol : '$') . ' ' . number_format((float)$order->discount,2,'.',',')}}</td>
              </tr>
              <tr>
                <th>Total:</th>
                <td class="text-right">{{ (currency() ? currency()->symbol : '$') . ' ' . number_format((float)$order->total,2,'.',',')}}</td>
              </tr>
              <tr>
                <th>Previously Paid:</th>
                <td class="text-right">{{ (currency() ? currency()->symbol : '$') . ' ' . number_format((float)($order->paid - $collection_amount),2,'.',',')}}</td>
              </tr>
              <tr>
                <th>Collection Amount:</th>
                <td class="text-right">{{ (currency() ? currency()->symbol : '$') . ' ' . number_format((float)$collection_amount,2,'.',',')}}</td>
              </tr>
              <tr>
                <th>Due:</th>
                <td class="text-right">{{ (currency() ? currency()->symbol : '$') . ' ' . number_format((float)$order->due,2,'.',',')}}</td>
              </tr>
            </table>
          </div>
        </div>
