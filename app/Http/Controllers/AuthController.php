...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if (!Auth::validate($request->only('email', 'password'))) {
                AuditService::logLogin($request->email, false, $request->ip());
            ...
            if (Auth::attempt($valid, $credentials['remember'])) {
                session()->regenerate();
                AuditService::logLogin($request->email, true, $request->ip());
                // Update last login IP on user record
    ...
    public function forgetPassword(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($findUser) {
                ...
                return redirect()->route('new.password')->with('success', 'Email verified. Please enter your new password.');
            } else {
    ...
    {
        if (!session('user_id')) {
            return redirect()->route('forget.password')->with('error', 'Please enter your email first.');
        }
        ...
        if ($request->isMethod('post')) {
            ...
            if ($user) {
                ...
                return redirect()->route('forget.password')->with('error', 'Something went wrong. Please try again.');
            }
        } else {
